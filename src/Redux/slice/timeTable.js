...
export const getTimeTableByDepartment = createAsyncThunk('timeTable/getTimeTableByDepartment', async (id) => {
  try {
    const res = await TimeTableServices.getTimeTableByDepartment(id);
    console.log("res==>", res)
    if (res.data) {
      toast.success(res.data.message);
    }
...
export const timeTabletSlice = createSlice({
  ...
  extraReducers: (builder) => {
    ...
    builder.addCase(getTimeTableByDepartment.pending, (state) => {
      state.loading = 'pending';
    });
    builder.addCase(getTimeTableByDepartment.fulfilled, (state, action) => {
      state.loading = 'idle';
      state.data = action.payload;
    });
    builder.addCase(getTimeTableByDepartment.rejected, (state, action) => {
      state.loading = 'rejected';
      state.message = action.payload
    });
