...
export default function ViewAttendence() {
  ...
  const user = useSelector((s) => s.attendence?.data?.attendance);
  const userData = useSelector((s) => s.user?.data?.user);

  console.log('attendence==>', userData);
  ...
  const getAllUsers = async (id) => {
    try {
      ...
      console.log('viewAttendence', res);
      if (res.payload.data) {
        setRefetch(true);
      }
    } catch (error) {
  ...
  useEffect(() => {
    getAllUsers(localStorage.getItem('ggscf_user_id'));
  }, [refetch, dispatch]);
      ...
      <Container>
        <Stack direction="row" alignItems="center" justifyContent="space-between" mb={5}>
          <Typography variant="h4" gutterBottom>
            Attendance of {userData.studentName}
          </Typography>
        </Stack>
              ...
              <Table>
                ...
                <TableBody>
                  {filteredUsers?.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage)?.map((row) => {
                    const { _id, studentName, status, date } = row;
